...
public class DownloadService
{
    ...
    private async Task<string> FetchJsonInternalAsync(string url, CancellationToken cancellationToken)
    {
        if (url == _cachedUrl && !string.IsNullOrWhiteSpace(_cachedJson))
            return _cachedJson;
    ...
    public async Task<VideoPreviewInfo?> GetVideoPreviewAsync(string url, CancellationToken cancellationToken = default)
    {
        try
        {
            var json = await FetchJsonInternalAsync(url, cancellationToken);
            using var doc = JsonDocument.Parse(json);
            var root = doc.RootElement;
            
            var title = root.TryGetProperty("title", out var titleProp) ? titleProp.GetString() ?? "" : "";
            var thumb = root.TryGetProperty("thumbnail", out var thumbProp) ? thumbProp.GetString() ?? "" : "";
            var uploader = root.TryGetProperty("uploader", out var upProp) ? upProp.GetString() ?? "" : "";
            var duration = root.TryGetProperty("duration_string", out var durProp) ? durProp.GetString() ?? "" : "";
        ...
        {
            return null; // Suppress errors for preview to fail gracefully
        }
    }

    public async Task<List<AudioFormatOption>> GetAudioFormatsAsync(string url, CancellationToken cancellationToken = default)
    {
        try
        {
            var json = await FetchJsonInternalAsync(url, cancellationToken);
    ...
    public async Task<(List<VideoFormatOption> VideoFormats, string BestAudioFormatId)> GetVideoFormatsAsync(string url, CancellationToken cancellationToken = default)
    {
        try
        {
            var json = await FetchJsonInternalAsync(url, cancellationToken);
    ...
    public async Task<List<ManualFormatOption>> GetAllFormatsAsync(string url, CancellationToken cancellationToken = default)
    {
        try
        {
            var json = await FetchJsonInternalAsync(url, cancellationToken);
    ...
    public async Task DownloadVideoAsync(string url, string videoFormatId, string audioFormatId, string outputPath, string container, IProgress<DownloadProgress> progress, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(settings.ProxyUrl)) args += $" --proxy \"{settings.ProxyUrl}\"";
